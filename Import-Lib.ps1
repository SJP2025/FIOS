...
$DrivePath = $PSScriptRoot
Write-Output "Sourcing Drive Path: ${DrivePath}"
#EndRegion

#Region Add \Lib\ PSModules Path
$InvokeLibFolder = "$DrivePath\Invoke-Lib"
$LibPath = "$DrivePath\Invoke-Lib"
...
if ($env:PSModulePath -notlike "*$InvokeLibFolder*") {
    try {
        ...
        Write-Host "Failed to add Invoke-Lib Module folder to PSModulePath: $_"
        exit 1
    }
} else {
    Write-Output "Invoke-Lib Module folder is already in PSModulePath."
}
#EndRegion
